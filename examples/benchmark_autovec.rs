...
fn main() {
    ...
    let _ = yuv_convert_libyuv::yuv420_to_rgb8(
        ...
        width, height, YuvRange::Full, YuvMatrix::Bt709);
    ...
    println!("Scalar:        {:.3} ms ({:.1} Mpixels/s)", scalar_ms, mpix / scalar_ms);
    println!("Auto-vec:      {:.3} ms ({:.1} Mpixels/s) - {:.2}x vs scalar", 
             autovec_ms, mpix / autovec_ms, scalar_ms / autovec_ms);
    
    if let Some(simd_ms) = simd_ms {
        println!("Manual SIMD:   {:.3} ms ({:.1} Mpixels/s) - {:.2}x vs scalar, {:.2}x vs autovec", 
                 simd_ms, mpix / simd_ms, scalar_ms / simd_ms, autovec_ms / simd_ms);
    }

...
- `zenav1-aom-encode` - EXPERIMENTAL `Av1Backend::Zenav1Aom` — zenav1-aom pure-Rust ENCODE backend via `aom_encode::key_frame::encode_key_frame` (git-rev dep on imazen/zenav1-aom at `41c9c065` — bumped 2026-09-15 from `fbea6b4` to the pushed `perf/gate3-txfm-i16-batch` head, carrying the `#[non_exhaustive]` `key_frame` surface, the `threads` row-band knob (serial default — byte-unchanged), `mode`/`screen_likelihood` (inert under the `LibaomExact` default this seam keeps), upstream palette+IntraBC defaults ON, and the bd8 u8 loop-filter kernels; earlier hops: `c3e1b4ab`→`45c53ddb` 2026-09-04 for the zenavif#45 fix `21544fde`, `45c53ddb`→`fbea6b4` for `ColorDescription`; a NEWER rev than the decode pin because `encode_key_frame` landed after it, and cargo resolves the two as two sources). **KEY-frame / still scope only** — that entry point encodes one frame and has no inter path, so animation is absent from the ENCODER, not merely unwired. Wired: RGB/RGBA → **4:4:4 or 4:2:0** at **8, 10 or 12 bits** in **either pixel range** (the `45c53ddb`-era refusal list below — alpha, 4:4:4, identity/RGB, full range, lossless — was all closed once `ColorDescription` existed; the alpha aux item is a mono FULL-range encode whose `matrix_coefficients` must be downgraded from the colour item's IDENTITY to unspecified — upstream refuses IDENTITY on monochrome). **Quality 100 = `--cq-level 0` is coded-lossless and gated EXACT** on every coded plane at 8/10/12 bits, both decoders (`aom_cq0_encodes_and_reconstructs_the_coded_planes_exactly`, with `cq0_gate_can_fail_on_a_lossy_encode` as its q99 mutation proof). It panicked in the port before the `45c53ddb` pin (zenavif#45, a `debug_assert` in `count_leaf`'s depth walk — fixed at the root in zenav1-aom `21544fde`); the canary `aom_cq0_still_panics_on_flat_content` that pinned it went red on the bump, as designed, and was retired into the gate. Still refused by name: animation, 4:2:2, identity outside 4:4:4, gain maps, high-bit-depth grayscale. Muxes in-crate via zenavif-serialize. Gates: `tests/aom_encode_backend.rs` (22 tests) + `tests/aom_roundtrip_loss.rs` (support-query/encode-path agreement in BOTH directions) + `tests/resolved_routing.rs`. See `src/encoder_aom.rs`.
...
decode cells under both real libaom and the in-repo decoder. At the current
pin `41c9c065` (2026-09-15) that gate is reported upstream as
**427/427** byte-identical (189 of them cq 0) plus
`coded_lossless_reconstructs_the_source_exactly` 248/248 — taken from the
issue record and zenav1-aom CI, not re-run in this checkout. The zenavif seam
does NOT claim byte parity with aomenc; it gates decodability, and at cq 0
exact reconstruction of the coded planes (`tests/aom_encode_backend.rs`). Scope is KEY-frame / still only — see the

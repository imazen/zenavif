...
fn test_yuv_conversion_accuracy() {
    ...
    for ((y, u, v), (exp_r, exp_g, exp_b)) in test_cases {
        ...
        yuv::yuv444_to_rgb(&planar, &mut rgb, 3, YuvRange::Full, YuvStandardMatrix::Bt601).unwrap();
        ...
        eprintln!("YUV({:3},{:3},{:3}) -> RGB({:3},{:3},{:3}) expected ({:3},{:3},{:3}) diff={}", 
                  y, u, v, rgb[0], rgb[1], rgb[2], exp_r, exp_g, exp_b, max_diff);
        
        // Allow up to 2 for rounding, but anything more is suspicious
        if max_diff > 2 {
            eprintln!("  WARNING: Difference {} exceeds rounding tolerance!", max_diff);
        }
    }
}

<!-- SALVAGED 2026-09-02 from `origin/abandoned/spike-av1-backends-2026-05-23`
     (commit 669de3c). Only the measurement records were brought onto `main`;
     the harness that produced them (`examples/bench_backends.rs`) and the
     backend-trait design it needed (`src/backend.rs`, `src/backend_ffmpeg.rs`)
     were NOT — that design was superseded by the `DecodeBackend` seam in
     `src/decode_av1.rs`, and the example cannot compile without it. To re-run
     this spike, read those three files out of commit 669de3c. -->

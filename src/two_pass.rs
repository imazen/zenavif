...
/// Options for the butteraugli-guided second pass.
///
...
pub struct TwoPassOptions {
    ...
    /// harder.
    pub strength: f64,
    /// Additive stabilizer on the raw `mse/butteraugli` ratio before
    ...
    /// Exponent of the per-superblock diffmap p-norm pool (libaom: 12 —
...
pub fn encode_rgb8_two_pass(
    ...
    // Butteraugli per-pixel difference map, source vs pass-1 decode.
    ...
    let ba = butteraugli::butteraugli(img, dec_img, &params).map_err(|e| {
        at!(Error::Encode(format!(
            "two-pass-butteraugli: diffmap computation failed: {e}"
        )))
    })?;
    let diffmap = ba.diffmap.as_ref().ok_or_else(|| {
        at!(Error::Encode(
            "two-pass-butteraugli: butteraugli returned no diffmap".to_string()
        ))
    })?;
    ...
    let sb_q_scale = pool_sb_q_scale(img, dec_img, diffmap.as_ref(), options);
    ...
    Ok(TwoPassEncode {
        ...
        pass1_butteraugli_max: ba.score,
        pass1_butteraugli_3n: ba.pnorm_3,
        sb_q_scale,
    })
}

/// Pools a butteraugli diffmap + pixel MSE per 64×64 superblock into AC
...
/// with negligible error on either signal stay neutral (`1.0`).
fn pool_sb_q_scale(
    src: ImgRef<'_, Rgb<u8>>,
    dec: ImgRef<'_, Rgb<u8>>,
    ...
    for sby in 0..sb_rows {
        for sbx in 0..sb_cols {
            ...
            let dbutteraugli = pool.powf(1.0 / exp);
            let dmse = sse / (px * 3.0);

            // libaom's eps guard: no reliable signal -> neutral.
            const EPS: f64 = 0.01;
            if dbutteraugli >= EPS && dmse >= EPS {
                let w_raw = (dmse / dbutteraugli).min(5.0) + options.k;

...
# zenrav1e--deltaq is the per-SB delta-q + variance-boost jj workspace
# (ravif's dev-only [patch.crates-io] targets it during the strength fit).
# ravif--wedge + zenrav1e--wedge are the WEDGE-FINDER program's PRIVATE clones
# (ravif@main + zenrav1e@master-tip with the dev-only tune/palette env
# passthroughs applied in the clone, NOT in the shared ravif tree — avoids
# contending for the ravif [patch.crates-io] slot with concurrent A/B sessions;
# run with CAVIF=/home/lilith/work/zen/ravif--wedge/target/release/cavif).
...
# ravif--coeffrd + zenrav1e--coeffrd are the COEFF_RD_STACK program's pair
# (docs/COEFF_RD_STACK.md; chain_coeffrd.sh): ravif@main private clone with
# tune/palette/ZENRAVIF_COEFFRD env passthroughs, path dep -> the
# zenrav1e--coeffrd jj workspace at master 3e5ff155 (the CoeffRdStack knob).
ZEN_REPOS=(ravif zenrav1e 'zenrav1e--drift-master?' 'zenrav1e--deltaq?' 'zenrav1e--qmdist?' 'zenrav1e--lfsharp?' 'ravif--wedge?' 'zenrav1e--wedge?' 'zenrav1e--sizedecay?' 'ravif--fastwins?' 'zenrav1e--fastwins?' 'ravif--p1part?' 'zenrav1e--p1part?' 'ravif--p2heads?' 'zenrav1e--p2heads?' 'ravif--s4tier?' 'zenrav1e--s4tier?' 'ravif--tuner2?' 'zenrav1e--tuner2?' 'ravif--ssimrd?' 'zenrav1e--ssimrd?' 'ravif--s10?' 'zenrav1e--s10?' 'zenavif--s10?' 'ravif--coeffrd?' 'zenrav1e--coeffrd?' zenavif zenanalyze fast-ssim2 zenpixels zencodec)
